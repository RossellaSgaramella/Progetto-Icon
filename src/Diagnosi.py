...
def leggi_da_file(carenza):
    file = open("../res/diagnosi-carenza-{}.txt".format(carenza), "r")
    return file.read()
...
class ExpertSystem(KnowledgeEngine):
    ...
    def _initial_action(self):
        ...
        yield Fact(cura_magnesio="Fertilizzare con concime idrosolubile a rapido assorbimento come il MgO 8%. I risultati si vedono in 2-4 giorni. Altra fonte di Magnesio (con Calcio, Ferro e altri microelementi) è il "
                                 "Cal-Mag Extra.")
    ...
    def regola_1(self):
        print("DIAGNOSI MMN")
        print("Un sistema esperto per riconoscere le carenze nutrizionali nelle coltivazioni, basandosi in una fase iniziale sui sintomi visivi presenti sulle foglie della pianta, e in una fase successiva sui valori \n"
              "raccolti da tessuto fogliare o dal terreno. ")
        self.declare(Fact(benvenuto="si"))

    @Rule(Fact(benvenuto="si"))
    def regola_2(self):
        self.declare(Fact(foglie_colpite=altre_risposte(
            "Sono colpite le foglie giovani o le foglie più vecchie", ["giovani", "vecchie", "entrambe"])))
